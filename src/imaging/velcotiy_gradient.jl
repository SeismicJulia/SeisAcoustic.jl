...
"""
   compute gradient for simultaneouse sources
"""
function velocity_gradient(res::Recordings, path_bnd::Ts, path_wfd::Ts,
...
"""
   get the sourceside wavefield generated by multi-sources
"""
function get_sourceside_wavefield(srcs::Vector{Source}, params::TdParams; iflag=1)
    ...
    # set the top to 0 if free surface is implemented
    if params.free_surface && iflag == 2
       pre[1,:,:] .= 0.0
    end
...
#     g = zeros(params.data_format, N)
#
#     # inject the recordings to the last snapshot (adjoint snapshot is ready)
#     inject_rec2spt!(spt2, res, params.nt)
#
#     # reconstruct sourceside wavefield at one step
#     subtract_source!(wfd2, src, params.nt)
#
#     # read one steo boundary value
#     fid_bnd = open(path_bnd, "r")
...
#
#     # compute the gradient
#     apply_image_condition!(g, spt2, wfd1, wfd2, params)
#
#     # prepare for the next step backward reconstruction
#     copy_wavefield!(wfd2, wfd1)
#
